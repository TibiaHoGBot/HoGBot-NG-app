...
local function createContext(webview, options)
  ...
  -- for key, val in pairs(functionMap) do
  --   print(key)
  -- end
        ...
        if fn then
          ...
          if kind == ';' then
            ...
            if s then
              ...
              handleCallback(callback, 'Fail to parse ' ..
                name .. ' JSON value "' .. tostring(value) .. '" due to ' .. tostring(r))
              return
            end
          end
...
local function parseArgs(args)
  ...
  if urlArg and urlArg ~= '' then
    if urlArg == '-h' or urlArg == '/?' or urlArg == '--help' then
      ...
      print('Optional arguments: url --wv-title= --wv-width=' ..
        tostring(width) .. ' --wv-height=' .. tostring(height) .. ' --wv-resizable=' .. tostring(resizable))
      os.exit(0)
    end
...
local function launchWithOptions(url, wvOptions, options)
  ...
  if options.eventMode then
    ...
    local webview = webviewLib.new(url, wvOptions.title, wvOptions.width, wvOptions.height, wvOptions.resizable,
      wvOptions.debug)
